...
def plot_dataset(x_ini):
    ...
    p.set_axis_off()
    plt.savefig(f'{DATASET_FOLDER}/areas/area_{x.name:05}.png',\
               dpi=dpi)   
    plt.close('all')
    ...
    plt.savefig(f'{DATASET_FOLDER}/buildings/building_{x.name:05}.png',
               dpi=dpi)   
    plt.close('all')
...
def load_images_from_path(path, idx):
    data = list()
    print(idx)
    for filename in sorted(listdir(path))[idx[0]:idx[1]]:
